  ...
  <div class="fillcontain">
    ...
    <div class="table_container">
      <el-row class="topFS">
        ...
        <el-select v-model="listinfo.Status" placeholder="请选择">
          <el-option v-for="(item, index) in optionsSelect" :label="item.label" :value="item.value" :key="index"></el-option>
        </el-select>
        <el-button type="primary" @click="OrderGetPage">搜 索</el-button>
      </el-row>
      <el-table
        v-loading="loading"
        :data="tableData"
    ...
    export default {
      data(){
        return {
          ...
          selectedCategory: [],
          count:0,
          choosetime: [new Date().setDate(new Date().getDate() - 1), new Date()],
          OrderCode:'',
          optionsSelect: [{
            value: '99',
            label: '全部'
      ...
      methods:{
        ...
        async OrderGetPage(){
          ...
          const restaurants = await OrderGetPage(form);
